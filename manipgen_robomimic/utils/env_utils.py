...
import manipgen_robomimic.envs.env_base as EB
...
def get_env_class(env_meta=None, env_type=None, env=None):
    ...
    if env_type == EB.EnvType.ROBOSUITE_TYPE:
        from manipgen_robomimic.envs.env_robosuite import EnvRobosuite
        return EnvRobosuite
    elif env_type == EB.EnvType.GYM_TYPE:
        from manipgen_robomimic.envs.env_gym import EnvGym
        return EnvGym
    elif env_type == EB.EnvType.IG_MOMART_TYPE:
        from manipgen_robomimic.envs.env_ig_momart import EnvGibsonMOMART
...
def set_env_specific_obs_processing(env_meta=None, env_type=None, env=None):
    ...
    if is_robosuite_env(env_meta=env_meta, env_type=env_type, env=env):
        from manipgen_robomimic.utils.obs_utils import DepthModality, process_frame, unprocess_frame
...
def wrap_env_from_config(env, config):
    ...
    if ("frame_stack" in config.train) and (config.train.frame_stack > 1):
        from manipgen_robomimic.envs.wrappers import FrameStackWrapper
