...
If you prefer to test the basic functionality of manipgen_robomimic without installing anything locally, try the quickstart [Colab notebook](https://colab.research.google.com/drive/1b62r_km9pP40fKF0cBdpdTO2P_2eIbC6?usp=sharing).
...
```sh
$ cd {/path/to/manipgen_robomimic}
```

### Step 1: Download dataset

Download the robosuite **Lift (PH)** dataset (see [this link](../datasets/manipgen_robomimic_v0.1.html#proficient-human-ph) for more information on this dataset):
```sh
$ python manipgen_robomimic/scripts/download_datasets.py --tasks lift --dataset_types ph
```
...
```sh
$ python manipgen_robomimic/scripts/train.py --config manipgen_robomimic/exps/templates/bc.json --dataset datasets/lift/ph/low_dim_v141.hdf5 --debug
```
...
This example [requires robosuite](./installation.html#install-simulators) to be installed (under the `v1.4.1` branch), but it can be run without robosuite by disabling rollouts in `manipgen_robomimic/exps/templates/bc.json`: simply change the `experiment.rollout.enabled` flag to `false`.
...
## Next steps
<!--
High-level overview of the `manipgen_robomimic` directory (highlighting selected files):
```
algo/                     # algorithms
...
scripts/                  # scripts
  ...
  download_datasets.py       # downloading manipgen_robomimic v0.1 datasets
