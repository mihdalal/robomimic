...
## Install sp_robomimic

<div class="admonition note">
<p class="admonition-title">1. Create and activate conda environment</p>

```sh
$ conda create -n sp_robomimic_venv python=3.8.0
$ conda activate sp_robomimic_venv
```
...
<div class="admonition note">
<p class="admonition-title">3. Install sp_robomimic</p>
...
$ git clone https://github.com/ARISE-Initiative/sp_robomimic.git
$ cd sp_robomimic
$ pip install -e .
```
...
If you would like to run sp_robomimic examples and work with released datasets, please install the following simulators:

<details>
  <summary><b>robosuite</b></summary>
<p>
 Required for running most sp_robomimic examples and released datasets. Compatible with robosuite v1.2+. Install via:
...
This assumes you have installed sp_robomimic from source.

Run a quick debugging (dummy) training loop to make sure sp_robomimic is installed correctly:
```sh
$ cd <PATH_TO_sp_robomimic_INSTALL_DIR>
...
```sh
$ cd <PATH_TO_sp_robomimic_INSTALL_DIR>/tests
$ bash test.sh
```
...
```sh
$ cd <PATH_TO_sp_robomimic_INSTALL_DIR>/docs
$ make clean
$ make apidoc
$ make html
