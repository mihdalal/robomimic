...
class DataLogger(object):
    ...
    def __init__(self, log_dir, config, log_tb=True, log_wandb=False):
        ...
        if log_wandb:
            import wandb
            import sp_robomimic.macros as Macros
            ...
            assert Macros.WANDB_ENTITY is not None, "WANDB_ENTITY macro is set to None." \
                    ...
                    "\nIf this file does not exist, first run python {base_path}/scripts/setup_macros.py".format(base_path=sp_robomimic.__path__[0])
...
def log_warning(message, color="yellow", print_now=True):
    ...
    buffer_message = colored("sp_robomimic WARNING(\n{}\n)".format(textwrap.indent(message, "    ")), color)
