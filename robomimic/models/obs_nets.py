...
class MIMO_Transformer_ACT(Module):
    """
    Extension to Transformer (based on GPT architecture) to accept multiple observation 
    dictionaries as input and to output dictionaries of tensors. Inputs are specified as 
    a dictionary of observation dictionaries, with each key corresponding to an observation group.
    ...
    def __init__(
        ...
        Args:
            ...
            encoder_kwargs (dict): observation encoder config
        """
        super(MIMO_Transformer_ACT, self).__init__()
        ...
        self.nets['action_encoder'] = obs_encoder_factory(
                obs_shapes=OrderedDict({'actions': [7]}),
                feature_activation=None,
                encoder_kwargs=encoder_kwargs,
            )
        ...
        self.nets["embed_obs_encoder"] = nn.Linear(
            transformer_input_dim, transformer_embed_dim
        )
    ...
    def forward(self, **inputs):
        ...
        transformer_encoder_outputs = None
        first_obs = {k: inputs['obs'][k][:, 0:1] for k in inputs['obs']}
        transformer_inputs_obs = TensorUtils.time_distributed(
            {'obs': first_obs}, self.nets["encoder"], inputs_as_kwargs=True
        )[:, 0]
        transformer_inputs_actions = TensorUtils.time_distributed(
            {'obs_dict': {'actions': inputs['actions'][:, :-1]}}, self.nets["action_encoder"], inputs_as_kwargs=True
        )
        transformer_inputs = dict(
            obs=transformer_inputs_obs.reshape(-1, len(self.nets['encoder'].observation_group_shapes['obs']),  transformer_inputs_obs.shape[-1]//len(self.nets['encoder'].observation_group_shapes['obs'])),
            actions=transformer_inputs_actions
        )
        transformer_inputs = torch.cat([transformer_inputs['obs'], transformer_inputs['actions']], dim=1)
        assert transformer_inputs.ndim == 3  # [B, T, D]
