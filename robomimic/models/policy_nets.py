...
from robomimic.models.obs_nets import MIMO_MLP, RNN_MIMO_MLP, MIMO_Transformer, MIMO_Transformer_ACT, ObservationDecoder
...
class TransformerActorACTNetwork(MIMO_Transformer_ACT):
    """
    An Transformer policy network that predicts actions from observation sequences (assumed to be frame stacked
    from previous observations) and possible from previous actions as well (in an autoregressive manner).
    """
    def __init__(
        ...
        output_shapes = self._get_output_shapes()
        super(TransformerActorACTNetwork, self).__init__(
            input_obs_group_shapes=observation_group_shapes,
            output_shapes=output_shapes,
            transformer_embed_dim=transformer_embed_dim,
            transformer_num_layers=transformer_num_layers,
    ...
    def forward(self, obs_dict, actions=None, goal_dict=None):
        ...
        if self._is_goal_conditioned:
            ...
            mod = list(obs_dict.keys())[0]
            goal_dict = TensorUtils.unsqueeze_expand_at(goal_dict, size=obs_dict[mod].shape[1], dim=1)
        forward_kwargs = dict(obs=obs_dict, goal=goal_dict, actions=actions)
        outputs = super(TransformerActorACTNetwork, self).forward(**forward_kwargs)
...
class TransformerGMMACTActorNetwork(TransformerActorACTNetwork):
    """
    A Transformer GMM policy network that predicts sequences of action distributions from observation 
    sequences (assumed to be frame stacked from previous observations).
    """
    def __init__(
        ...
        super(TransformerGMMACTActorNetwork, self).__init__(
            obs_shapes=obs_shapes,
            ac_dim=ac_dim,
            transformer_embed_dim=transformer_embed_dim,
            transformer_num_layers=transformer_num_layers,
    ...
    def forward_train(self, obs_dict, actions=None, goal_dict=None, low_noise_eval=None):
        ...
        outputs = MIMO_Transformer_ACT.forward(self, **forward_kwargs)
        means = outputs["mean"][:, len(obs_dict)-1:]
        scales = outputs["scale"][:, len(obs_dict)-1:]
        logits = outputs["logits"][:, len(obs_dict)-1:]
